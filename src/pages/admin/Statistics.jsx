  ...
  return (
    <div className="admin-page">
      <div className="admin-header" style={{ marginBottom: '24px' }}>
        <h2 style={{ fontSize: '24px', color: '#1e293b' }}>Statistics & Analytics</h2>
      </div>
      ...
      <div className="stats-row" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '30px' }}>
        <div className="admin-stat-card" style={{ background: '#ffffff', padding: '20px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
          <div style={{ color: '#64748b', fontSize: '14px', marginBottom: '8px' }}>Total Organizations</div>
          <div style={{ color: '#1e293b', fontSize: '28px', fontWeight: 'bold' }}>{stats.totalOrganizations}</div>
        </div>
        <div className="admin-stat-card" style={{ background: '#ffffff', padding: '20px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
          <div style={{ color: '#64748b', fontSize: '14px', marginBottom: '8px' }}>Avg Vacancies per Job</div>
          <div style={{ color: '#1e293b', fontSize: '28px', fontWeight: 'bold' }}>{stats.avgVacancies}</div>
        </div>
        <div className="admin-stat-card" style={{ background: '#ffffff', padding: '20px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
          <div style={{ color: '#64748b', fontSize: '14px', marginBottom: '8px' }}>Most Active Category</div>
          <div style={{ color: '#1e293b', fontSize: '20px', fontWeight: 'bold', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{stats.mostActiveCategoryName}</div>
          <div style={{ color: '#10b981', fontSize: '12px', marginTop: '4px' }}>{stats.mostActiveCategoryCount} jobs</div>
        </div>
        <div className="admin-stat-card" style={{ background: '#ffffff', padding: '20px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
          <div style={{ color: '#64748b', fontSize: '14px', marginBottom: '8px' }}>Jobs This Month</div>
          <div style={{ color: '#1e293b', fontSize: '28px', fontWeight: 'bold' }}>{stats.jobsThisMonth}</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '30px' }}>
        ...
        {/* Jobs by Category */}
        <div className="admin-chart-card" style={{ background: '#ffffff', padding: '24px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
          <h3 style={{ color: '#1e293b', marginBottom: '20px', fontSize: '16px' }}>Jobs by Category</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            {stats.categoryStats.filter(c => c.count > 0).map(cat => (
              <div key={cat.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                <span style={{ width: '120px', fontSize: '13px', color: '#64748b', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{cat.nameEn}</span>
                ...
                <span style={{ width: '40px', fontSize: '13px', fontWeight: '600', color: '#1e293b', textAlign: 'right' }}>{cat.count}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {/* Jobs by Type */}
          <div className="admin-chart-card" style={{ background: '#ffffff', padding: '24px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
            <h3 style={{ color: '#1e293b', marginBottom: '20px', fontSize: '16px' }}>Jobs by Type</h3>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              {Object.entries(stats.typeCounts).map(([type, count], idx) => {
                ...
                return (
                  <div key={type} style={{ background: '#f8fafc', padding: '16px', borderRadius: '8px' }}>
                    <div style={{ color: '#64748b', fontSize: '13px', marginBottom: '8px' }}>{type}</div>
                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: '8px', marginBottom: '8px' }}>
                      <span style={{ color: '#1e293b', fontSize: '24px', fontWeight: 'bold' }}>{count}</span>
          ...
          {/* Salary Distribution */}
          <div className="admin-chart-card" style={{ background: '#ffffff', padding: '24px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
            <h3 style={{ color: '#1e293b', marginBottom: '20px', fontSize: '16px' }}>Salary Range Distribution</h3>
                ...
                return (
                  <div key={item.label}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                      ...
                      <span style={{ color: '#1e293b', fontSize: '13px', fontWeight: '600' }}>{item.count} ({perc}%)</span>
                    </div>
      ...
      <div className="admin-chart-card" style={{ background: '#ffffff', padding: '24px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
        <h3 style={{ color: '#1e293b', marginBottom: '20px', fontSize: '16px' }}>Jobs by Location</h3>
        <div className="admin-table-wrapper" style={{ overflowX: 'auto', background: '#ffffff', borderRadius: '8px' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
                <th style={{ padding: '12px 16px', color: '#64748b', fontSize: '13px', fontWeight: '500' }}>Location</th>
                <th style={{ padding: '12px 16px', color: '#64748b', fontSize: '13px', fontWeight: '500' }}>Job Count</th>
                <th style={{ padding: '12px 16px', color: '#64748b', fontSize: '13px', fontWeight: '500' }}>Percentage</th>
                <th style={{ padding: '12px 16px', color: '#64748b', fontSize: '13px', fontWeight: '500', width: '250px' }}>Distribution</th>
              </tr>
            </thead>
            <tbody>
              {stats.locationStats.length > 0 ? (
                stats.locationStats.slice(0, 10).map((loc, idx) => (
                  <tr key={loc.location} style={{ borderBottom: idx !== stats.locationStats.length - 1 ? '1px solid #e2e8f0' : 'none' }}>
                    <td style={{ padding: '12px 16px', color: '#1e293b', fontSize: '14px' }}>{loc.location}</td>
                    <td style={{ padding: '12px 16px', color: '#1e293b', fontSize: '14px' }}>{loc.count}</td>
                    <td style={{ padding: '12px 16px', color: '#1e293b', fontSize: '14px' }}>{loc.percentage}%</td>
